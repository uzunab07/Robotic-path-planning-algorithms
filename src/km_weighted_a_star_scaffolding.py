...
def weighted_astar_search(graph, start, goal,weight=-1):
    # # -- print the graph
    # print ("Graph: ", graph)
    # sys.exit()
    ...
    fScore[start] = heuristic(start,goal) * weight
    ...
    while not open_set or not path_found:
        ...
        for neighbor, cost in graph[current_node]:
            ...
            if tentative_gScore < gScore[neighbor]:
                ...
                fScore[neighbor] = gScore[neighbor] + weight * heuristic(neighbor, goal)  # Adjust heuristic with weight
...
def main(filename = os.getcwd()+'/resources/grid_0.txt'):
    ...
    path = weighted_astar_search(graph, start, goal)
    # -- plot the path
    plot_path(grid_env, path, 'Weighted A* Search')
    
if __name__ == '__main__':
    absolute_path = os.getcwd()
    filename = '/resources/grid_4.txt'
